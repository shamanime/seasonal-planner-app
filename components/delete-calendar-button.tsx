  ...
  return (
    <>
      ...
      <dialog ref={dialogRef} className="w-[min(92vw,28rem)] rounded-[1.5rem] p-0 shadow-card backdrop:bg-ink/55">
        <form action={deleteFamilyCalendar} className="rounded-[1.5rem] bg-white p-6 text-ink">
          ...
          <div className="mt-6 flex flex-wrap justify-end gap-3">
            <button className="rounded-full bg-white px-4 py-2 text-sm font-bold shadow-sm" type="button" onClick={() => dialogRef.current?.close()}>
              Cancel
            </button>
            <button
