...
test("a family can clone, customize, and navigate its calendar", async ({
	page,
}) => {
	await page.goto("/");
	...
	await signIn(page);
	await expect(
		page.getByRole("heading", { name: "Your seasonal calendars" }),
	).toBeVisible();
	...
	await addActivity
		...
		.fill("Added from Playwright");
	await addActivity
		.getByRole("button", { name: "Add custom activity" })
		.click();
